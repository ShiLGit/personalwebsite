...
projRouter.route("/delete/:docID/:projID").delete(auth, async (req, res) => {
	let err = null;
	const deleted = await ProjText.findByIdAndDelete(req.params.docID).catch((e) => {
		console.log(e);
		err = e;
	});
	...
	if (!err) return res.status(200).send({ success: "Successfully deleted " + deleted.projName, deleted });
