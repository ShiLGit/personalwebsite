...
class EditProj extends Component {
	...
	componentDidUpdate() {
		...
		if (this.props.curProjID !== this.state.curProjID) {
			...
			if (this.props.curProjID === null) this.clearCurProj();
		}
	}
	...
	identifierChangeHandler = (e) => {
		...
		for (let i = 0; i < id.length; i++) {
			if (id[i].match(allowedChars)) parsedID += id[i];
		}

		//reformat image names to match projID
		if (this.state.demoImageName.indexOf(this.state.projID) === -1 && this.state.projID.length > 0) {
			console.log(
				`([${this.state.projID}], [${parsedID}]), [${
					this.state.demoImageName
				}], FINAL:  ${this.state.demoImageName.replace(this.state.projID, parsedID)}`
			);
	...
	editProj = () => {
		...
		//edit project
		axios
			.put(BASE_REQ_URL + "/projects/edittext/" + this.state.curProjID, payload, {
	...
	addProj = (fData) => {
		...
		//upload pictures
		axios
			.post(BASE_REQ_URL + "/projects/addpic", fData, { headers: { Authorization: `${this.props.token}` } })
			.then((res) => {
				this.setState({ loading: false });
			})
			.catch((e) => {
		...
		axios
			.post(BASE_REQ_URL + "/projects/addtext", payload, {
				headers: { Authorization: `${this.props.token}` }
			})
			.then((res) => {
				alert(res.data.success);
				let update = res.data.saved;
				if (update.curProjID) delete update.curProjID;
		...
		if (!this.state.curProjID) {
			...
			if (this.state.icon.type.includes("image")) {
				...
				fData.append(
					"pictures",
					this.state.icon,
					imgName + "_icon" + this.state.icon.type.replace("image/", ".")
				);
				fData.append(
					"pictures",
					this.state.demoImage,
					imgName + "_demo" + this.state.demoImage.type.replace("image/", ".")
				);
			} else {
				this.setState({ loading: false });
	...
	render() {
		...
		if (!this.props.token) toRender = <Redirect to='/unauthorized' />;

		const toggleAddProj = (
			<button style={{ position: "absolute", top: "0px", right: "0px" }} onClick={this.clearCurProj}>
				Add Project
			</button>
		);
		...
		return (
			<React.Fragment>
				{toRender}
				{this.state.loading ? (
					<Loader style={{ marginTop: "30vh" }} />
				) : (
					<form
						className={styles.Form}
						style={{ marginTop: "90px", minWidth: "fit-content", position: "relative" }}
						onSubmit={this.onSubmitHandler}
						encType='multipart/form-data'
					>
						{this.props.curProjID ? toggleAddProj : null}
						<h2 style={{ textAlign: "center", marginTop: "-10px", display: "inline-block" }}>
							{this.props.curProjID ? "Edit " + this.props.curProjID : "Add Project"}
						</h2>
						...
						<input
							...
							required={required}
							disabled={!required}
							onChange={this.identifierChangeHandler}
						...
						<input
							type='text'
							required
							onChange={this.projChangeHandler}
						...
						<input
							type='text'
							required
							onChange={this.titleDescChangeHandler}
						...
						<input
							...
							style={{ margin: "auto" }}
							required={required}
							onChange={this.demoImageChangeHandler}
						></input>
						<br />
